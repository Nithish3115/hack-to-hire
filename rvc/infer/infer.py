    ...
    def convert_audio(
        ...
        **kwargs,
        ...
        try:
            ...
            print(f"Input audio shape: {audio.shape}, max: {np.max(audio)}, min: {np.min(audio)}")
            if np.isnan(audio).any():
                print("WARNING: NaN detected in input audio!")
            ...
            for c in chunks:
                ...
                if np.isnan(audio_opt).any():
                    print(f"WARNING: NaN detected in output of pipeline for chunk {len(converted_chunks)+1}!")
            ...
            print(f"Output audio shape: {audio_opt.shape}, max: {np.max(audio_opt)}, min: {np.min(audio_opt)}")
            if np.isnan(audio_opt).any():
                print("WARNING: NaN detected in final output audio!")
